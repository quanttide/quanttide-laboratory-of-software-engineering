...
| 产出 | 场景 | 评分 | 说明 |
|------|------|------|------|
| `parts[2]` 越界分析 | 单函数安全 | **A** | 引用行号、路径准确、建议可操作 |
| `trim()` 重复识别 | 单函数模式 | **A** | 精确识别两种粒度的重复 |
| `sum += v` 误报 | finding 审查 | **A+** | LLM 纠正了规则引擎的误报 |
| `process_order` 职责 | 单函数架构 | **C** | 缺乏行号引用，修复建议是通用模板 "职责不够单一" |
| 正式项目根因 | 跨文件 | **D** | 引用空白、结论错误、建议空洞 |

**规律：** reflect 在小范围（单函数、已知问题）上表现很好，一旦跨文件或跨模块推理质量急剧下降。根源是 LLM 在没有精确证据时容易滑向"架构师空话"。

**验证有效的条件：**
1. 输入必须包含精确的行号、变量名、数据流路径
2. 分析范围 ≤ 单函数
3. LLM 的结论可以被机械工具的结果直接支撑

**验证无效的条件：**
1. 跨文件、跨模块的根因分析——LLM 产出无法验证的空洞建议
2. 没有精确证据链的开放问题
