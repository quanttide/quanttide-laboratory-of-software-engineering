...
本文档为 Agent 在 qtcloud-code-cli 实验室（`examples/default`）中工作提供指南。
...
backward_slice 与 dataflow：从数组访问处追溯变量来源，交由 LLM 发现越界风险。

### 模式 2 重复模式识别

flatten_stmts 展平语句，交由 LLM 识别重复的解析与验证模式。

### 模式 3 一致性检查

dataflow 追踪多个变量路径，交由 LLM 交叉对比发现错误传播问题。
